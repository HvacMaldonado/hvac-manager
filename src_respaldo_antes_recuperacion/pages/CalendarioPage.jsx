...
function getTechnicianTheme(tecnicos = [], tecnicoId) {
  ...
  return TECHNICIAN_THEMES[index % TECHNICIAN_THEMES.length];
}
...
export default function CalendarioPage({
  ...
  const events = useMemo(() => {
    const citaEvents = citas.map((cita) => {
      ...
      const theme = getTechnicianTheme(tecnicos, cita.tecnicoId);
    ...
    const ordenEvents = ordenes
      ...
      .map((orden) => {
        ...
        const theme = getTechnicianTheme(tecnicos, orden.tecnicoId);
    ...
    <section className="space-y-4">
      ...
      <div className="rounded-3xl border border-slate-200 bg-white/95 p-4 shadow-xl shadow-slate-300/50">
        ...
        <div className="flex flex-wrap gap-2">
          {tecnicos.map((tec) => {
            const theme = getTechnicianTheme(tecnicos, tec.id);
