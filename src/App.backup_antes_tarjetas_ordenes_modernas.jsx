    ...
    <div className="min-h-screen w-full max-w-full overflow-x-hidden bg-[radial-gradient(circle_at_top_left,_#0f172a55,_transparent_30%),radial-gradient(circle_at_top_right,_#0284c733,_transparent_28%),radial-gradient(circle_at_bottom_left,_#1e40af2e,_transparent_30%),linear-gradient(135deg,_#e2e8f0_0%,_#cbd5e1_48%,_#94a3b8_100%)] text-slate-900">
      <header className="sticky top-0 z-20 border-b border-slate-900/10 bg-slate-950/90 text-white backdrop-blur-xl shadow-2xl shadow-slate-900/30">
        <div className="w-full px-3 2xl:px-8 py-2.5 2xl:py-4 flex flex-col 2xl:flex-row 2xl:items-center 2xl:justify-between gap-2.5 2xl:gap-4">
          <div><p className="text-[10px] 2xl:text-xs uppercase tracking-[0.24em] 2xl:tracking-[0.3em] text-cyan-300 font-black">{t("app")}</p><h1 className="text-lg 2xl:text-lg font-black tracking-tight text-white">{session.role === "admin" ? t("adminPanel") : `${t("techPanel")}: ${session.nombre}`}</h1></div>
      ...
      <main className="w-full min-w-0 px-2.5 2xl:px-6 py-3 2xl:py-5">
        ...
        {session.role === "admin" && (
          <>
            ...
            <nav className="sticky top-[66px] 2xl:top-[92px] z-10 mb-3 2xl:mb-3 2xl:mb-5 flex gap-1 overflow-x-auto 2xl:flex-wrap rounded-2xl border border-slate-700/30 bg-slate-950/85 p-1.5 shadow-lg 2xl:shadow-2xl shadow-slate-900/25 backdrop-blur [-webkit-overflow-scrolling:touch]">
          ...
          return (
            <>
              <StatsBar
                ...
                inventario={inventario}
                herramientas={herramientasTecnico}
              />

              <section className="mb-3 overflow-hidden rounded-2xl border border-white/70 bg-white/95 shadow-md shadow-slate-300/50">
                <div className="bg-gradient-to-br from-slate-950 via-blue-950 to-cyan-900 px-4 py-3 text-white">
                  <p className="text-[9px] font-black uppercase tracking-[0.24em] text-cyan-300">
                    Panel del técnico
                  </p>
                  <h2 className="mt-0.5 text-xl font-black">{session.nombre}</h2>
                </div>

                <div className="grid gap-2 bg-slate-50 p-2 sm:grid-cols-3">
                  <button
                    onClick={() => setTecnicoVista("agenda")}
                    className={
                      "rounded-2xl px-4 py-3 text-left transition-all " +
                      (tecnicoVista === "agenda"
                        ? "bg-gradient-to-br from-cyan-600 via-blue-700 to-slate-950 text-white shadow-lg"
                  ...
                  >
                    <div className="flex items-center justify-between gap-3">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm font-black">Agenda de hoy</span>
                      </div>
                  ...
                  <button
                    ...
                    className={
                      ...
                      (tecnicoVista === "ordenes"
                        ? "bg-gradient-to-br from-indigo-600 via-blue-700 to-slate-950 text-white shadow-lg"
                        : "bg-white text-slate-700 ring-1 ring-slate-200 hover:bg-slate-100")
                    }
                  >
                    <div className="flex items-center justify-between gap-3">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm font-black">Órdenes</span>
                      </div>
                      <span className="text-2xl font-black">{ordenesActivasTecnico.length}</span>
                    </div>
                    <p className="mt-1 text-[11px] font-bold opacity-80">
                      Tarjetas de órdenes activas
                    </p>
                  </button>
                  ...
                  <button
                    ...
                    className={
                      ...
                      (tecnicoVista === "historial"
                        ? "bg-gradient-to-br from-emerald-600 via-green-700 to-slate-950 text-white shadow-lg"
                        : "bg-white text-slate-700 ring-1 ring-slate-200 hover:bg-slate-100")
                    }
                  >
                    <div className="flex items-center justify-between gap-3">
                      <div className="flex items-center gap-2">
                        <History size={20} />
                        <span className="text-sm font-black">Historial</span>
                      </div>
...
function SoftStat({ titulo, valor, icon: Icon, accent = "from-slate-950 to-blue-900" }) {
  return (
    <div className={`relative min-w-[118px] overflow-hidden rounded-2xl border border-white/10 bg-gradient-to-br ${accent} px-3 py-2.5 text-white shadow-md shadow-slate-900/20`}>
      ...
      <p className="relative z-10 mt-2 truncate text-[10px] font-black uppercase tracking-wide text-slate-100">
        {titulo}
      </p>
    </div>
  );
...
function TecnicoOrderGroup({ title, subtitle, icon: Icon, tone, ordenes, obtenerCliente, ordenProps }) {
  ...
  return (
    <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white/95 shadow-md shadow-slate-300/50">
      <div className={`flex flex-col gap-2 px-4 py-3 text-white sm:flex-row sm:items-center sm:justify-between ${tone}`}>
        <div>
...
function ReprogramCitaModal({ open, cita, cliente, onClose, onConfirm }) {
  ...
  return (
    <div className="fixed inset-0 z-[980] flex items-center justify-center bg-slate-950/75 p-3 backdrop-blur-sm">
      <div className="w-full max-w-xl overflow-hidden rounded-3xl border border-white/20 bg-white shadow-2xl shadow-slate-950/50">
        <header className="flex items-center justify-between bg-gradient-to-br from-blue-950 via-cyan-800 to-teal-700 px-5 py-3 text-white">
          <div>
        ...
        <main className="space-y-4 bg-[radial-gradient(circle_at_top_right,_#22d3ee33,_transparent_28%),linear-gradient(135deg,_#eff6ff_0%,_#ecfeff_55%,_#f8fafc_100%)] p-5">
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            <label className="block">
              ...
              <input
                ...
                className="w-full rounded-2xl border border-slate-300 bg-white p-3 text-sm font-bold outline-none focus:border-cyan-500 focus:ring-4 focus:ring-cyan-100"
              />
            </label>
          ...
          <label className="block">
            ...
            <textarea
              ...
              className="min-h-28 w-full rounded-2xl border border-slate-300 bg-white p-3 text-sm outline-none focus:border-cyan-500 focus:ring-4 focus:ring-cyan-100"
            />
          </label>
          ...
          <div className="flex flex-col gap-2 sm:flex-row sm:justify-end">
            ...
            <button onClick={confirmar} className="rounded-2xl bg-gradient-to-r from-blue-700 to-cyan-700 px-4 py-3 text-sm font-black text-white shadow-lg">
...
function CancelOrderModal({ open, orden, cliente, tecnico, onClose, onConfirm, session }) {
  ...
  return (
    <div className="fixed inset-0 z-[980] flex items-center justify-center bg-slate-950/75 p-3 backdrop-blur-sm">
      <div className="w-full max-w-xl overflow-hidden rounded-3xl border border-white/20 bg-white shadow-2xl shadow-slate-950/50">
        <header className="flex items-center justify-between bg-gradient-to-br from-rose-950 via-red-800 to-orange-700 px-5 py-3 text-white">
          <div>
...
function TecnicoAssignedTodayPanel({ ordenes = [], citas = [], obtenerCliente }) {
  ...
  return (
    <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white/95 shadow-md shadow-slate-300/50">
      <div className="border-b border-slate-200 bg-gradient-to-r from-slate-950 via-blue-950 to-cyan-900 px-5 py-3 text-white">
        ...
        <div className="max-h-[440px] overflow-y-auto bg-slate-50 p-4">
          <div className="relative space-y-4">
            ...
            {items.map((item, index) => {
              ...
              const dotClass = isOrden
                ? "bg-blue-600 ring-blue-100"
                : "bg-amber-500 ring-amber-100";

              const cardClass = isOrden
                ? "border-blue-200 bg-blue-50"
                : "border-amber-200 bg-amber-50";

              const badgeClass = isOrden
                      ...
                      <div className="flex flex-wrap gap-2 xl:justify-end">
                        {item.direccion && (
                          <a
                            ...
                            target="_blank"
                            rel="noreferrer"
                            className="inline-flex items-center gap-2 rounded-2xl bg-slate-950 px-3 py-2 text-xs font-black text-white shadow-sm"
                          >
                            <Navigation size={15} />
                            Ir
                          </a>
                        ...
                        {item.telefono && (
                          <a
                            ...
                            className="inline-flex items-center gap-2 rounded-2xl bg-emerald-600 px-3 py-2 text-xs font-black text-white shadow-sm"
                          >
                            <Phone size={15} />
                            Llamar
                          </a>
  ...
  return (
    <section className="mb-6 space-y-4">
      <div className="overflow-hidden rounded-3xl border border-white/70 bg-white/95 shadow-xl shadow-slate-300/60 backdrop-blur">
        <div className="bg-gradient-to-br from-slate-950 via-blue-950 to-cyan-900 px-5 py-3 text-white">
          <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
            <div>
              <p className="text-[10px] font-black uppercase tracking-[0.28em] text-cyan-300">Agenda del técnico</p>
              <h2 className="mt-1 flex items-center gap-2 text-lg font-black">
            ...
            <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
              ...
              <div className="rounded-2xl bg-white/10 px-3 py-2 text-center ring-1 ring-white/20">
                ...
                <p className="text-[9px] font-black uppercase tracking-wide text-cyan-200">Órdenes hoy</p>
              </div>
              <div className="rounded-2xl bg-white/10 px-3 py-2 text-center ring-1 ring-white/20">
                <p className="text-lg font-black">{totalCitasHoy}</p>
                <p className="text-[9px] font-black uppercase tracking-wide text-cyan-200">Citas hoy</p>
              </div>
              <div className="rounded-2xl bg-white/10 px-3 py-2 text-center ring-1 ring-white/20">
      ...
      {ordenes.length === 0 && citas.length === 0 && (
        <div className="rounded-3xl border border-slate-200 bg-white/95 p-8 text-center text-sm font-semibold text-slate-500 shadow-xl shadow-slate-300/50">
      ...
      {vista === "todas" ? (
        ...
        <>
          ...
          <TecnicoOrderGroup
            ...
            icon={Clock3}
            tone="bg-gradient-to-r from-slate-950 via-blue-950 to-cyan-900"
...
function OrdenCard({ orden, cliente, inventario, obtenerMaterial, obtenerTecnico, colorEstado, colorPrioridad, marcarEnRuta, iniciarTrabajo, marcarNecesitaSeguimiento, completarOrden, cancelarOrden, subirFoto, guardarNotaTecnico, urlGoogleMaps, urlAppleMaps, urlTelefono, compacta = false, agregarMaterialAOrden, actualizarMaterialOrden, eliminarMaterialOrden, calcularCostoOrden, materialesTexto, compartirOrden, t }) {
  ...
  if (compacta) return <div className="rounded-2xl 2xl:rounded-3xl border border-slate-200 bg-white p-3 2xl:p-4 shadow-sm"><div className="grid grid-cols-1 xl:grid-cols-[1fr_auto] gap-2 2xl:gap-3"><div><p className="font-black text-slate-950">{cliente?.nombre || "Cliente eliminado"}</p><p className="text-sm text-slate-600">{orden.problema}</p><p className="text-[11px] text-slate-500">Técnico: {tecnico?.nombre || ""}</p><p className="mt-1 text-sm font-bold text-emerald-700">Completada: {orden.fechaCompletada ? new Date(orden.fechaCompletada).toLocaleDateString() : ""}</p><p className="text-xs text-slate-500">Horas: {orden.duracionHoras || "0.00"} | Materiales: {materialesTexto(orden) || "Sin materiales"}</p></div><ShareButtons orden={orden} compartirOrden={compartirOrden} t={t} /></div></div>;
  return <div className="w-full max-w-full min-w-0 rounded-2xl border border-white/70 bg-white p-3 shadow-md shadow-slate-200/70"><div className="mb-2 flex justify-between gap-3"><div><p className="text-sm font-black">{cliente?.nombre || "Cliente eliminado"}</p><p className="text-xs text-slate-600">{orden.problema}</p><p className="text-[11px] text-slate-500">Técnico: {tecnico?.nombre || "Sin asignar"}</p><p className="text-[11px] text-slate-500">{telefono}</p><p className="text-[11px] text-slate-500">{direccion}</p></div><span className={`h-fit rounded-full border px-2.5 py-1 text-[11px] font-bold ${colorEstado(orden.estado)}`}>{orden.estado}</span></div><div className="mb-2 flex flex-wrap gap-1.5"><a href={urlAppleMaps(direccion)} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 rounded-xl bg-gradient-to-r from-cyan-600 to-blue-600 px-2.5 py-1.5 text-[11px] font-bold text-white shadow-md shadow-cyan-100"><Navigation {...iconProps} />Apple Maps</a><a href={urlTelefono(telefono)} className="inline-flex items-center gap-2 rounded-xl bg-emerald-600 px-2.5 py-1.5 text-[11px] font-bold text-white"><Phone {...iconProps} />{t("call")}</a></div><div className="mb-2 grid grid-cols-2 sm:grid-cols-5 gap-1.5 text-[11px]"><Info icon={AlertTriangle} titulo="Prioridad" valor={orden.prioridad} extra={colorPrioridad(orden.prioridad)} /><Info icon={Calendar} titulo="Fecha" valor={orden.fecha} /><Info icon={Clock3} titulo="Inicio" valor={orden.horaInicio ? new Date(orden.horaInicio).toLocaleTimeString([], { hour: "numeric", minute: "2-digit", hour12: true }) : "Sin iniciar"} /><Info icon={Clock4} titulo="Cierre" valor={orden.horaCierre ? new Date(orden.horaCierre).toLocaleTimeString([], { hour: "numeric", minute: "2-digit", hour12: true }) : "Sin cerrar"} /><Info icon={Timer} titulo="Horas" valor={orden.duracionHoras || "0.00"} /></div><Materiales orden={orden} inventario={inventario} agregarMaterialAOrden={agregarMaterialAOrden} actualizarMaterialOrden={actualizarMaterialOrden} eliminarMaterialOrden={eliminarMaterialOrden} /><div className="mb-2 rounded-2xl border bg-slate-50 p-2.5"><p className="mb-2 flex items-center gap-2 text-sm font-black"><Camera {...iconProps} />{t("photos")}</p><div className="grid grid-cols-3 gap-2"><FotoUploader titulo="Antes" imagen={orden.fotos?.antes} onChange={(archivo) => subirFoto(orden.id, "antes", archivo)} /><FotoUploader titulo="Durante" imagen={orden.fotos?.durante} onChange={(archivo) => subirFoto(orden.id, "durante", archivo)} /><FotoUploader titulo="Después" imagen={orden.fotos?.despues} onChange={(archivo) => subirFoto(orden.id, "despues", archivo)} /></div></div><label className="mb-2 flex items-center gap-2 text-sm font-black"><FileText {...iconProps} />{t("notes")}</label><textarea value={orden.notasTecnico || ""} onChange={(e) => guardarNotaTecnico(orden.id, e.target.value)} placeholder="Detalles del trabajo realizado..." className="mb-2 min-h-16 w-full rounded-xl border p-2 text-xs outline-none focus:ring-4 focus:ring-blue-100" /><div className="flex flex-wrap gap-2"><button onClick={() => iniciarTrabajo(orden.id)} className="inline-flex items-center gap-1.5 rounded-xl bg-blue-600 px-2.5 py-1.5 text-[11px] font-bold text-white"><LoaderCircle {...iconProps} />{t("start")}</button><button onClick={() => completarOrden(orden.id)} className="inline-flex items-center gap-2 rounded-xl bg-emerald-600 px-2.5 py-1.5 text-[11px] font-bold text-white"><CheckCircle2 {...iconProps} />{t("complete")}</button><button onClick={() => marcarNecesitaSeguimiento(orden.id)} className="inline-flex items-center gap-1.5 rounded-xl bg-amber-600 px-2.5 py-1.5 text-[11px] font-bold text-white"><AlertTriangle {...iconProps} />No completada</button><button onClick={() => cancelarOrden(orden)} className="inline-flex items-center gap-1.5 rounded-xl bg-rose-600 px-2.5 py-1.5 text-[11px] font-bold text-white"><Ban {...iconProps} />Cancelar orden</button></div></div>;
}
function Info({ titulo, valor, extra = "bg-white", icon: Icon = FileText }) { return <div className={`rounded-xl border p-2 text-xs ${extra}`}><p className="flex items-center gap-2 text-slate-400"><Icon {...iconProps} />{titulo}</p><p className="truncate font-bold">{valor}</p></div>; }
function Materiales({ orden, inventario, agregarMaterialAOrden, actualizarMaterialOrden, eliminarMaterialOrden }) { return <div className="mb-2 rounded-2xl border bg-slate-50 p-2.5"><div className="mb-3 flex items-center justify-between"><p className="flex items-center gap-2 font-black"><PackageOpen {...iconProps} />Material usado</p><button onClick={() => agregarMaterialAOrden(orden.id)} className="inline-flex items-center gap-2 rounded-xl 2xl:rounded-2xl bg-purple-600 px-2 2xl:px-3 py-1.5 2xl:py-2 text-xs 2xl:text-sm font-bold text-white"><Plus {...iconProps} />Agregar material</button></div>{(orden.materialesUsados || []).length === 0 && <p className="text-[11px] text-slate-500">No se ha agregado material.</p>}<div className="space-y-2">{(orden.materialesUsados || []).map((m) => <div key={m.id} className="grid grid-cols-1 sm:grid-cols-[minmax(0,1fr)_80px_auto] 2xl:grid-cols-[minmax(0,1fr)_120px_auto] gap-2 items-center"><select value={m.inventarioId} onChange={(e) => actualizarMaterialOrden(orden.id, m.id, "inventarioId", e.target.value)} className="rounded-xl 2xl:rounded-2xl border p-1.5 2xl:p-2 text-sm 2xl:text-base"><option value="">Seleccionar material</option>{inventario.map((i) => <option key={i.id} value={i.id}>{i.nombre} ({i.cantidad} {i.unidad})</option>)}</select><input type="number" value={m.cantidad} onChange={(e) => actualizarMaterialOrden(orden.id, m.id, "cantidad", e.target.value)} placeholder="Cantidad" className="rounded-xl 2xl:rounded-2xl border p-1.5 2xl:p-2 text-sm 2xl:text-base" /><button onClick={() => eliminarMaterialOrden(orden.id, m.id)} className="inline-flex items-center gap-2 rounded-2xl px-3 py-2 text-sm font-bold text-red-600"><Trash2 {...iconProps} />Quitar</button></div>)}</div></div>; }
function FotoUploader({ titulo, imagen, onChange }) { return <div className="rounded-xl 2xl:rounded-2xl border bg-white p-2 2xl:p-3 text-sm 2xl:text-base"><p className="mb-2 text-sm font-bold">{titulo}</p>{imagen ? <img src={imagen} alt={titulo} className="mb-2 h-20 w-full rounded-xl border object-cover" /> : <div className="mb-2 flex h-20 w-full items-center justify-center rounded-xl border border-dashed bg-slate-50 text-sm text-slate-400">Sin foto</div>}<input type="file" accept="image/*" onChange={(e) => onChange(e.target.files[0])} className="w-full text-xs" /></div>; }
function ShareButtons({ orden, compartirOrden, t }) {
  return (
    <div className="flex max-w-full flex-wrap gap-1.5 2xl:gap-2 h-fit">
      <button onClick={() => compartirOrden(orden, "imprimir")} className="inline-flex items-center gap-2 rounded-xl 2xl:rounded-2xl bg-gradient-to-r from-slate-950 to-blue-900 px-2 2xl:px-3 py-1.5 2xl:py-2 text-xs 2xl:text-sm font-bold text-white shadow-md shadow-slate-300">
        <Printer {...iconProps} />
        Imprimir
      </button>

      <button onClick={() => compartirOrden(orden, "mensaje")} className="inline-flex items-center gap-2 rounded-xl 2xl:rounded-2xl bg-blue-800 px-2 2xl:px-3 py-1.5 2xl:py-2 text-xs 2xl:text-sm font-bold text-white">
        <Share2 {...iconProps} />
        Compartir
      </button>

      <button onClick={() => compartirOrden(orden, "whatsapp")} className="inline-flex items-center gap-2 rounded-xl 2xl:rounded-2xl bg-emerald-700 px-2 2xl:px-3 py-1.5 2xl:py-2 text-xs 2xl:text-sm font-bold text-white">
        <MessageCircle {...iconProps} />
        WhatsApp
      </button>

      <button onClick={() => compartirOrden(orden, "messenger")} className="inline-flex items-center gap-2 rounded-xl 2xl:rounded-2xl bg-sky-700 px-2 2xl:px-3 py-1.5 2xl:py-2 text-xs 2xl:text-sm font-bold text-white">
        <Send {...iconProps} />
        Messenger
      </button>

      <button onClick={() => compartirOrden(orden, "email")} className="inline-flex items-center gap-2 rounded-xl 2xl:rounded-2xl bg-slate-100 px-2 2xl:px-3 py-1.5 2xl:py-2 text-xs 2xl:text-sm font-bold">
        <Mail {...iconProps} />
        {t("email")}
      </button>
    </div>
      ...
      <div className="overflow-hidden rounded-3xl border border-white/70 bg-white/95 shadow-xl shadow-slate-300/60 backdrop-blur">
        ...
        <div className="grid gap-2 border-b border-slate-200 bg-white p-3 sm:grid-cols-3">
          <div className="rounded-2xl bg-slate-950 px-4 py-3 text-white">
            ...
            <p className="mt-1 text-lg font-black">{activas}</p>
          </div>
          <div className="rounded-2xl bg-emerald-50 px-4 py-3 text-emerald-800 ring-1 ring-emerald-100">
            ...
            <p className="mt-1 text-lg font-black">{completadas}</p>
          </div>
          <div className="rounded-2xl bg-rose-50 px-4 py-3 text-rose-800 ring-1 ring-rose-100">
            <p className="text-xs font-black uppercase tracking-wide text-rose-600">Canceladas</p>
            <p className="mt-1 text-lg font-black">{canceladas}</p>
          </div>
        </div>
          ...
          {tabs.map((item) => {
            ...
            return (
              <button
                ...
                className={
                  "inline-flex shrink-0 items-center gap-2 rounded-xl px-3 py-2 text-xs font-black transition " +
                  (active
  ...
  return (
    <section className="space-y-4">
      <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white/95 shadow-md shadow-slate-300/50">
        <div className="bg-gradient-to-br from-slate-950 via-emerald-950 to-cyan-900 px-5 py-3 text-white">
          ...
          <p className="mt-1 text-sm font-semibold text-slate-300">
            Órdenes completadas y canceladas, separadas de tu trabajo activo.
          </p>
        </div>

        <div className="grid gap-3 border-b border-slate-200 bg-white p-3 sm:grid-cols-3">
          <div className="rounded-2xl bg-slate-950 px-4 py-3 text-white">
            <p className="text-xs font-black uppercase tracking-wide text-slate-300">Todas</p>
            <p className="mt-1 text-lg font-black">{ordenes.length}</p>
          </div>

          <div className="rounded-2xl bg-emerald-50 px-4 py-3 text-emerald-800 ring-1 ring-emerald-100">
            <p className="text-xs font-black uppercase tracking-wide text-emerald-600">Completadas</p>
            <p className="mt-1 text-lg font-black">{completadas.length}</p>
          </div>

          <div className="rounded-2xl bg-rose-50 px-4 py-3 text-rose-800 ring-1 ring-rose-100">
            <p className="text-xs font-black uppercase tracking-wide text-rose-600">Canceladas</p>
            <p className="mt-1 text-lg font-black">{canceladas.length}</p>
          </div>
        </div>
          ...
          {ordenesFiltradas.map((orden) => {
            ...
            return (
              <article
                key={orden.id}
                className={
                  "overflow-hidden rounded-2xl border bg-white shadow-md shadow-slate-200/70 " +
                  (completada ? "border-emerald-200" : "border-rose-200")
                }
              >
                <div
                  className={
                    "flex flex-col gap-2 px-4 py-3 text-white sm:flex-row sm:items-center sm:justify-between " +
                    (completada
                      ? "bg-gradient-to-r from-emerald-700 via-green-700 to-slate-950"
                      : "bg-gradient-to-r from-rose-700 via-red-700 to-slate-950")
                  }
                >
                  <div>
                    <p className="text-[10px] font-black uppercase tracking-[0.25em] text-white/70">
                      {completada ? "Orden completada" : "Orden cancelada"}
                    </p>
                ...
                <div className="grid gap-3 p-3 lg:grid-cols-[1fr_auto]">
                  <div className="min-w-0 space-y-3">
                    ...
                    <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
                      ...
                      <div className="rounded-xl bg-slate-50 p-2.5">
                        <p className="text-[10px] font-black uppercase text-slate-400">Materiales</p>
                        <p className="mt-1 text-sm font-black text-slate-800">
                          ${Number(orden.costoMateriales || 0).toFixed(2)}
                        </p>
                      </div>
                    ...
                    {!completada && (
                      <div className="rounded-2xl bg-rose-50 p-3 text-rose-800 ring-1 ring-rose-100">
                        <p className="text-[10px] font-black uppercase">Motivo de cancelación</p>
                        <p className="mt-1 text-sm font-bold">
                          {orden.cancelReason || "Sin motivo registrado"}
                        </p>
                      </div>
                    )}

                    {orden.notasTecnico && (
                      <div className="rounded-2xl bg-blue-50 p-3 text-blue-900 ring-1 ring-blue-100">
                        <p className="text-[10px] font-black uppercase">Notas del técnico</p>
                        <p className="mt-1 text-sm font-bold">{orden.notasTecnico}</p>
                      </div>
                    )}
                  </div>
                  ...
                  <div className="flex flex-wrap gap-2 lg:w-44 lg:flex-col">
                    {cliente?.telefono && (
                      <a
                        ...
                        className="inline-flex items-center justify-center gap-2 rounded-2xl bg-emerald-600 px-4 py-3 text-sm font-black text-white shadow-sm"
                      >
                        <Phone size={16} />
                        Llamar
                      </a>
                    ...
                    {ordenProps?.compartirOrden && (
                      <button
                        ...
                        className="inline-flex items-center justify-center gap-2 rounded-2xl bg-blue-600 px-4 py-3 text-sm font-black text-white shadow-sm"
                      >
                        <Printer size={16} />
                        Imprimir
                      </button>
