...
function Metric({ icon: Icon, label, value, tone }) {
  return (
    <div className={`flex min-w-[135px] items-center gap-3 rounded-2xl bg-gradient-to-br ${tone} px-4 py-3 text-white shadow-sm`}>
      <Icon size={18} />
      <div>
...
export default function ReportesDashboardPage({ clientes, ordenes, inventario, herramientas, tecnicos, obtenerTecnico, exportarCSV }) {
  ...
  const exportDashboard = () => {
    const rows = [
      ...
      ...data.tecnicoStats.map((t) => ({ Tipo: "Rendimiento técnico", Nombre: t.tecnico, Total: t.total, Completadas: t.completadas, Canceladas: t.canceladas, Efectividad: `${t.efectividad}%` })),
  ...
  return (
    <section className="space-y-4">
      <div className="overflow-hidden rounded-3xl border border-slate-200 bg-white/95 shadow-xl shadow-slate-300/60 backdrop-blur">
        <div className="bg-gradient-to-br from-slate-950 via-blue-950 to-cyan-900 p-5 text-white">
          <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
            <div>
              <p className="text-[10px] font-black uppercase tracking-[0.28em] text-cyan-300">Dashboard ejecutivo</p>
            ...
            <div className="flex flex-wrap items-center gap-2">
              <button onClick={exportDashboard} className="inline-flex items-center gap-2 rounded-2xl bg-white/10 px-4 py-3 text-sm font-black text-white ring-1 ring-white/20">
                <FileSpreadsheet size={16} />
                Exportar Excel
              </button>
              <button onClick={imprimirDashboard} className="inline-flex items-center gap-2 rounded-2xl bg-cyan-400 px-4 py-3 text-sm font-black text-slate-950">
                <Printer size={16} />
                Imprimir/PDF
              </button>
            </div>
        ...
        <div className="grid gap-3 bg-[radial-gradient(circle_at_top_right,_#22d3ee33,_transparent_28%),linear-gradient(135deg,_#f8fafc_0%,_#eff6ff_48%,_#e0f7ff_100%)] p-4 lg:grid-cols-[1fr_220px] print:hidden">
      ...
      <div className="grid gap-4 xl:grid-cols-2">
        <section className="rounded-3xl border border-slate-200 bg-white/95 p-4 shadow-xl shadow-slate-300/50">
        ...
        <section className="rounded-3xl border border-slate-200 bg-white/95 p-4 shadow-xl shadow-slate-300/50 xl:col-span-2">
          <h3 className="mb-4 flex items-center gap-2 text-xl font-black text-slate-950">
            <Package size={20} />
