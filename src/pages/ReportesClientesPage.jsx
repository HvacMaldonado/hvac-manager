...
function activityStatus(totalOrdenes, citasCount) {
  if (totalOrdenes > 0) return "Con historial";
  if (citasCount > 0) return "Con cita";
  return "Nuevo";
}

function statusClass(status) {
  if (status === "Con historial") return "border-emerald-200 bg-emerald-50 text-emerald-700";
  if (status === "Con cita") return "border-blue-200 bg-blue-50 text-blue-700";
...
export default function ReportesClientesPage({ t, clientes, ordenes, citas = [], obtenerCliente, exportarCSV, urlAppleMaps, urlTelefono }) {
  ...
  const filas = useMemo(() => {
    ...
    let rows = clientes.map((c) => {
      ...
      const status = activityStatus(ordenesC.length, citasC.length);
      ...
      <div className="overflow-hidden rounded-3xl border border-slate-200 bg-white/95 shadow-xl shadow-slate-300/60 backdrop-blur">
          ...
          <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-300">
                Resumen por cliente con órdenes, citas, actividad y acciones rápidas.
              </p>
            </div>

            <div className="flex flex-wrap items-center gap-2 xl:justify-end">
              <Metric icon={Users} label="Clientes" value={clientes.length} tone="from-slate-950 to-blue-900" />
              <Metric icon={ClipboardList} label="Órdenes" value={totalOrdenes} tone="from-blue-800 to-cyan-700" />
              <Metric icon={TrendingUp} label="Con historial" value={clientesConHistorial} tone="from-emerald-800 to-teal-600" />
              <Metric icon={CalendarDays} label="Con citas" value={clientesConCitas} tone="from-cyan-800 to-blue-700" />
            </div>
          </div>
        </div>
        ...
        <div className="grid gap-3 bg-[radial-gradient(circle_at_top_right,_#22d3ee33,_transparent_28%),linear-gradient(135deg,_#f8fafc_0%,_#eff6ff_48%,_#e0f7ff_100%)] p-4 lg:grid-cols-[1fr_180px_180px_auto]">
          ...
          <select value={filtro} onChange={(e) => setFiltro(e.target.value)} className="rounded-2xl border border-slate-300 bg-white px-3 py-3 text-sm font-bold text-slate-700 outline-none shadow-sm focus:border-blue-700 focus:ring-4 focus:ring-blue-100">
            ...
            <option value="conOrdenes">Con órdenes</option>
            <option value="conCitas">Con citas</option>
              ...
              {filas.map((c) => (
                <article key={c.id} className="grid grid-cols-[1.2fr_0.9fr_1.2fr_0.7fr_0.7fr_0.8fr_260px] gap-3 px-4 py-3 text-sm transition hover:bg-blue-50/50">
                  <div className="min-w-0">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="min-w-0">
                        ...
                        <span className={`mt-1 inline-flex rounded-full border px-2 py-0.5 text-[10px] font-black ${statusClass(c.status)}`}>
                          {c.status}
                        </span>
                      </div>
                    </div>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[10px] font-bold text-slate-500">Activas {c.activas}</span>
                  </div>
