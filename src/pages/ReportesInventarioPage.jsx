...
function StockBadge({ status, t }) {
  ...
  const Icon = status === t("stockLow") || status === "Dañada" || status === "Perdida" ? AlertTriangle : CheckCircle2;
...
export default function ReportesInventarioPage({ t, inventario, herramientas, obtenerTecnico, exportarCSV }) {
  ...
  const filas = useMemo(() => {
    const inventarioRows = inventario.map((i) => {
      ...
      const status = cantidad <= stockMinimo ? t("stockLow") : t("available");

      return {
        id: `inv-${i.id}`,
        tipo: t("inventoryShort"),
        nombre: i.nombre || t("noName"),
        categoria: i.categoria || t("noCategory"),
        cantidad,
        unidad: i.unidad || t("unit"),
        costo,
        total: cantidad * costo,
        estado: status,
        responsable: t("warehouse"),
      };
    });

    const herramientaRows = herramientas.map((h) => {
      const estado = h.estado || t("available");
      return {
        id: `tool-${h.id}`,
        tipo: t("tool"),
        nombre: h.nombre || t("noName"),
        categoria: t("technicalTool"),
    ...
    rows = rows.filter((row) => {
      ...
      const matchEstado =
        estadoFiltro === "todos" ||
        (estadoFiltro === "alertas" && [t("stockLow"), "Dañada", "Perdida"].includes(row.estado)) ||
    ...
    return rows.sort((a, b) => {
      const alertaA = [t("stockLow"), "Dañada", "Perdida"].includes(a.estado) ? 1 : 0;
      const alertaB = [t("stockLow"), "Dañada", "Perdida"].includes(b.estado) ? 1 : 0;
  ...
  const alertasHerramientas = herramientas.filter((h) => [t("damaged"), t("lost")].includes(h.estado)).length;
      ...
      <div className="overflow-hidden rounded-3xl border border-slate-200 bg-white/95 shadow-xl shadow-slate-300/60 backdrop-blur">
        <div className="bg-gradient-to-br from-slate-950 via-blue-950 to-cyan-900 p-5 text-white">
          <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
            <div>
              <p className="text-[10px] font-black uppercase tracking-[0.28em] text-cyan-300">{t("reports")}</p>
              <h2 className="mt-1 flex items-center gap-2 text-2xl font-black">
                <FileSpreadsheet size={24} />
                {t("inventoryReportsTitle")}
              </h2>
              <p className="mt-1 text-sm text-slate-300">
                {t("inventoryReportsDescription")}
              </p>
            </div>

            <div className="flex flex-wrap items-center gap-2 xl:justify-end">
              <Metric icon={Package} label={t("inventoryShort")} value={totalInventario} tone="from-slate-950 to-blue-900" />
              <Metric icon={Wrench} label={t("tools")} value={totalHerramientas} tone="from-blue-800 to-cyan-700" />
              <Metric icon={AlertTriangle} label={t("alerts")} value={stockBajo + alertasHerramientas} tone="from-amber-700 to-orange-600" />
              <Metric icon={Layers3} label={t("categories")} value={categorias} tone="from-cyan-800 to-blue-700" />
              <Metric icon={TrendingUp} label={t("value")} value={`$${valorInterno.toFixed(2)}`} tone="from-emerald-800 to-teal-600" />
            </div>
          </div>
        </div>
        ...
        <div className="grid gap-3 bg-[radial-gradient(circle_at_top_right,_#22d3ee33,_transparent_28%),linear-gradient(135deg,_#f8fafc_0%,_#eff6ff_48%,_#e0f7ff_100%)] p-4 lg:grid-cols-[1fr_170px_190px_auto]">
          <div className="relative">
            ...
            <input
              ...
              placeholder={t("searchInventoryReport")}
          ...
          <select value={tipoFiltro} onChange={(e) => setTipoFiltro(e.target.value)} className="rounded-2xl border border-slate-300 bg-white px-3 py-3 text-sm font-bold text-slate-700 outline-none shadow-sm focus:border-blue-700 focus:ring-4 focus:ring-blue-100">
            ...
            <option value="Inventario">{t("inventoryShort")}</option>
            <option value="Herramienta">{t("tools")}</option>
          </select>
          ...
          <select value={estadoFiltro} onChange={(e) => setEstadoFiltro(e.target.value)} className="rounded-2xl border border-slate-300 bg-white px-3 py-3 text-sm font-bold text-slate-700 outline-none shadow-sm focus:border-blue-700 focus:ring-4 focus:ring-blue-100">
            ...
            <option value="alertas">{t("onlyAlerts")}</option>
            <option value="Disponible">{t("available")}</option>
            <option value="Asignada">{t("assignedStatus")}</option>
            <option value="Devuelta">Devuelta</option>
            <option value="Dañada">Dañada</option>
            <option value="Perdida">Perdida</option>
            <option value="Stock bajo">{t("stockLow")}</option>
          </select>
                ...
                <article key={row.id} className="grid grid-cols-[0.8fr_1.2fr_1.1fr_0.65fr_0.8fr_0.85fr_0.9fr_0.9fr] gap-3 px-4 py-3 text-sm transition hover:bg-blue-50/50">
                  <div className="flex items-center">
                    <span className={`inline-flex items-center gap-1.5 rounded-full px-2 py-1 text-xs font-black ${
                      row.tipo === t("inventoryShort") ? "bg-blue-50 text-blue-700" : "bg-cyan-50 text-cyan-700"
                    }`}>
                      {row.tipo === t("inventoryShort") ? <Boxes size={13} /> : <Wrench size={13} />}
                      {row.tipo}
                    </span>
                  </div>
                  ...
                  <div className="flex items-center">
                    <StockBadge status={row.estado} t={t} />
                  </div>
                  ...
                  <div className="flex items-center">
                    <p className="font-black text-slate-950">{row.tipo === t("inventoryShort") ? `$${row.total.toFixed(2)}` : "—"}</p>
                  </div>
                </article>
              ))}
            </div>
