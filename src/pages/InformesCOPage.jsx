...
export default function InformesCOPage({
  lang = "es",
  informes = [],
  ordenes = [],
  abrirInforme,
  ...
  const textos = {
    eyebrow: en ? "SAFETY DOCUMENTS" : "DOCUMENTOS DE SEGURIDAD",
    title: en ? "CO Inspection Reports" : "Informes de inspección CO",
    description: en
      ? "Search, review, print and email carbon monoxide reports."
      : "Busca, consulta, imprime y envía informes de monóxido de carbono.",
    search: en
    ...
    view: en ? "View" : "Ver",
    noReports: en
      ? "No CO reports match the selected filters."
      : "No hay informes CO con los filtros seleccionados.",
  ...
  return (
    <section className="space-y-3">
      <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-md shadow-slate-200/70">
        <div className="bg-gradient-to-r from-slate-950 via-red-950 to-orange-900 px-4 py-3 text-white">
        ...
        <div className="grid gap-2 bg-slate-50 p-2 lg:grid-cols-[minmax(0,1fr)_auto]">
          <label className="flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 shadow-sm">
            <Search size={15} className="shrink-0 text-red-700" />
            <input
              value={busqueda}
              onChange={(event) =>
          ...
          <div className="grid grid-cols-3 gap-1 rounded-xl bg-red-50 p-1">
            {[
              ...
              <div className="divide-y divide-slate-200">
                {filas.map((informe) => {
                  ...
                  return (
                    <article
                      key={informe.id}
                      className="grid grid-cols-[150px_1.15fr_1.7fr_1fr_120px_105px_90px] items-center gap-3 px-4 py-2.5 text-xs transition hover:bg-red-50/60"
                    >
                      ...
                      <div className="flex justify-end">
                        <button
                          ...
                          title={textos.view}
                          className="inline-flex h-8 items-center justify-center gap-1.5 rounded-lg bg-red-700 px-3 text-[11px] font-black text-white shadow-sm transition hover:bg-red-600"
                        >
                          <Eye size={13} />
                          {textos.view}
                        </button>
