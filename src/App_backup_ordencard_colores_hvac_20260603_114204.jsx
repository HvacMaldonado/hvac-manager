...
<body>
  <main class="page">
    <header class="header">
      <div class="brand">
        <div class="logo">
          <img src="/logo-hvac-maldonado.jpg" alt="HVAC Refrigeración Maldonado R" />
        </div>
        <div>
...
const compartirOrden = async (orden, metodo) => {
  ...
  return (
    <div className="min-h-screen w-full max-w-full overflow-x-hidden bg-slate-100 text-slate-900">
      <header className="sticky top-0 z-20 border-b pl-44 border-slate-900/10 bg-slate-950/90 text-white backdrop-blur-xl shadow-lg shadow-slate-300/40"><img src="/logo-hvac-maldonado.jpg" alt="HVAC Refrigeración Maldonado R" className="absolute left-3 top-1/2 h-24 w-auto -translate-y-1/2 rounded-xl bg-white px-2 py-1 shadow-md" />
          ...
          <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
            <div className="min-w-0">
              <div className="flex flex-wrap items-center gap-1.5">
                <span className="rounded-full bg-white/15 px-3 py-1 text-[10px] font-black uppercase tracking-wide text-white ring-1 ring-white/20">
                  Orden de trabajo
                </span>
              ...
              <p className="mt-3 line-clamp-2 rounded-2xl bg-white/10 px-3 py-2 text-sm font-bold leading-relaxed text-white/95">
                {orden.problema || "Sin problema reportado"}
              </p>

              <div className="mt-3 grid gap-2 text-[11px] font-bold text-cyan-100 sm:grid-cols-2">
                <p className="flex items-center gap-1.5 truncate rounded-xl bg-white/10 px-2.5 py-1.5 ring-1 ring-white/10">
                  <User size={13} />
                ...
                {direccion && (
                  <p className="flex items-center gap-1.5 truncate rounded-xl bg-white/10 px-2.5 py-1.5 ring-1 ring-white/10 sm:col-span-2">
                    <MapPin size={13} />
                    {direccion}
                  </p>
                )}
            ...
            <div className="flex flex-wrap gap-2 lg:justify-end">
              {telefono && (
                <a href={urlTelefono(telefono)} className="flex items-center justify-center gap-1.5 rounded-2xl bg-emerald-600 px-4 py-3 text-sm font-black text-white shadow-lg shadow-emerald-900/20 transition hover:-translate-y-0.5">
                  <Phone size={14} />
                  Llamar
                </a>
              )}
              ...
              {direccion && (
                <a href={urlAppleMaps(direccion)} target="_blank" rel="noreferrer" className="flex items-center justify-center gap-1.5 rounded-2xl bg-white px-4 py-3 text-sm font-black text-slate-950 shadow-lg transition hover:-translate-y-0.5">
                  <Navigation size={14} />
                  Mapa
                </a>
              )}

              <button onClick={() => setVerDetalles(!verDetalles)} className="flex items-center justify-center gap-1.5 rounded-2xl bg-blue-600 px-4 py-3 text-sm font-black text-white shadow-lg shadow-blue-900/20 transition hover:-translate-y-0.5">
                <FileText size={14} />
          ...
          {verDetalles && (
            <div className="m-4 space-y-4 rounded-[1.5rem] border border-slate-200 bg-white p-4 shadow-inner shadow-slate-100">
              <div className="grid grid-cols-2 gap-2 text-xs sm:grid-cols-3 xl:grid-cols-6">
                ...
                <Info icon={Timer} titulo="Trabajo" valor={`${orden.duracionHoras || "0.00"} h`} />
                <Info icon={Navigation} titulo="Traslado" valor={`${orden.duracionTraslado || "0.00"} h`} />
              </div>
              ...
              <textarea
                ...
                className="min-h-24 w-full rounded-2xl border border-slate-200 bg-white p-3 text-sm font-semibold outline-none transition focus:border-blue-400 focus:ring-4 focus:ring-blue-100"
              />
...
function Info({ titulo, valor, extra = "bg-white", icon: Icon = FileText }) {
  return (
    <div className={`rounded-[1.35rem] border border-blue-100 bg-white p-4 text-sm shadow-md shadow-blue-100/40 ${extra}`}>
      <p className="flex items-center gap-2 text-[11px] font-black uppercase tracking-wide text-blue-500">
        <Icon {...iconProps} />
        {titulo}
      </p>
      <p className="mt-2 truncate text-base font-black text-slate-950">{valor}</p>
    </div>
  );
}
...
function Materiales({ orden, inventario, agregarMaterialAOrden, actualizarMaterialOrden, eliminarMaterialOrden }) {
  return (
    <div className="mb-2 rounded-[1.75rem] border border-purple-100 bg-gradient-to-br from-white via-purple-50/50 to-blue-50 p-5 shadow-lg shadow-purple-100/60">
      <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <p className="flex items-center gap-2 text-xl font-black text-slate-950">
          <PackageOpen {...iconProps} />
          Material usado
        </p>

        <button
          onClick={() => agregarMaterialAOrden(orden.id)}
          className="flex items-center justify-center gap-2 rounded-2xl bg-purple-600 px-4 py-3 text-sm font-black text-white shadow-lg shadow-purple-200 transition hover:-translate-y-0.5"
        >
          <Plus {...iconProps} />
          Agregar material
        </button>
      ...
      <div className="space-y-3">
        {(orden.materialesUsados || []).map((m) => (
          <div
            key={m.id}
            className="grid grid-cols-1 items-center gap-3 rounded-2xl border border-purple-100 bg-white p-3 shadow-sm sm:grid-cols-[minmax(0,1fr)_110px_auto]"
          >
            <select
              value={m.inventarioId}
              onChange={(e) => actualizarMaterialOrden(orden.id, m.id, "inventarioId", e.target.value)}
              className="rounded-2xl border border-slate-200 bg-white p-3 text-sm font-bold outline-none focus:border-purple-400 focus:ring-4 focus:ring-purple-100"
            >
            ...
            <button
              ...
              className="flex items-center justify-center gap-2 rounded-2xl border border-red-100 bg-red-50 px-4 py-3 text-sm font-black text-red-600"
            >
              <Trash2 {...iconProps} />
              Quitar
            </button>
...
function FotoUploader({ titulo, imagen, onChange }) {
  return (
    <div className="rounded-[1.5rem] border border-blue-100 bg-white p-4 text-sm shadow-md shadow-blue-100/50">
      ...
      {imagen ? (
        ...
        <div className="mb-3 flex h-32 w-full items-center justify-center rounded-2xl border-2 border-dashed border-blue-300 bg-blue-50 text-sm font-black text-blue-500">
          Sin foto
        </div>
      )}
