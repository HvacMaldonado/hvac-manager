...
function OrdenCard({ orden, cliente, inventario, obtenerMaterial, obtenerTecnico, colorEstado, colorPrioridad, marcarEnRuta, iniciarTrabajo, marcarNecesitaSeguimiento, completarOrden, cancelarOrden, subirFoto, guardarNotaTecnico, urlGoogleMaps, urlAppleMaps, urlTelefono, compacta = false, agregarMaterialAOrden, actualizarMaterialOrden, eliminarMaterialOrden, calcularCostoOrden, materialesTexto, compartirOrden, t }) {
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-3 shadow-sm">
        <div className="grid grid-cols-1 gap-2 xl:grid-cols-[1fr_auto]">
          <div>
            ...
            <p className="text-sm text-slate-600">{orden.problema}</p>
            <p className="text-xs text-slate-500">Horas: {orden.duracionHoras || "0.00"} | Materiales: {materialesTexto(orden) || "Sin materiales"}</p>
          </div>
          <ShareButtons orden={orden} compartirOrden={compartirOrden} t={t} />
        </div>
      </div>
    );
  }
  ...
  return (
    <article className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-md shadow-slate-200/70">
      <div className="grid gap-3 p-3 md:grid-cols-[96px_minmax(0,1fr)_auto] md:items-start">
        <div className="rounded-2xl bg-slate-950 px-3 py-2 text-center text-white">
          <p className="text-base font-black leading-tight">{horaTexto}</p>
          <p className="mt-1 text-[9px] font-black uppercase tracking-wide text-slate-300">
            #{orden.id}
          </p>
        </div>
        ...
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-1.5">
            <span className={`rounded-full border px-2.5 py-1 text-[11px] font-black ${colorEstado(orden.estado)}`}>
              {orden.estado}
            </span>
            <span className={`rounded-full border px-2.5 py-1 text-[11px] font-black ${colorPrioridad(orden.prioridad)}`}>
              {orden.prioridad}
            </span>
          </div>
        ...
        <div className="flex flex-wrap gap-1.5 md:justify-end">
          {telefono && (
            <a href={urlTelefono(telefono)} className="inline-flex items-center gap-1.5 rounded-xl bg-emerald-600 px-2.5 py-1.5 text-[11px] font-black text-white">
              <Phone size={14} />
              Llamar
            </a>
          )}
          ...
          {direccion && (
            <a href={urlAppleMaps(direccion)} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1.5 rounded-xl bg-slate-950 px-2.5 py-1.5 text-[11px] font-black text-white">
              <Navigation size={14} />
              Mapa
            </a>
          )}

          <button onClick={() => setVerDetalles(!verDetalles)} className="inline-flex items-center gap-1.5 rounded-xl bg-blue-600 px-2.5 py-1.5 text-[11px] font-black text-white">
            <FileText size={14} />
      ...
      {orden.estado === "Necesita seguimiento" && orden.seguimientoMotivo && (
        <div className="mx-3 mb-3 rounded-2xl border border-amber-200 bg-amber-50 p-2 text-xs font-bold text-amber-900">
      ...
      {verDetalles && (
        <div className="border-t border-slate-200 bg-slate-50 p-3">
          ...
          <div className="mb-2 rounded-2xl border bg-white p-2.5">
          ...
          <textarea
            ...
            className="mb-2 min-h-16 w-full rounded-xl border bg-white p-2 text-xs outline-none focus:ring-4 focus:ring-blue-100"
          />

          <div className="flex flex-wrap gap-2">
            <button onClick={() => iniciarTrabajo(orden.id)} className="inline-flex items-center gap-1.5 rounded-xl bg-blue-600 px-2.5 py-1.5 text-[11px] font-bold text-white">
              <LoaderCircle {...iconProps} />
              {t("start")}
            </button>

            <button onClick={() => completarOrden(orden.id)} className="inline-flex items-center gap-1.5 rounded-xl bg-emerald-600 px-2.5 py-1.5 text-[11px] font-bold text-white">
              <CheckCircle2 {...iconProps} />
              {t("complete")}
            </button>

            <button onClick={() => marcarNecesitaSeguimiento(orden.id)} className="inline-flex items-center gap-1.5 rounded-xl bg-amber-600 px-2.5 py-1.5 text-[11px] font-bold text-white">
              <AlertTriangle {...iconProps} />
              No completada
            </button>

            <button onClick={() => cancelarOrden(orden)} className="inline-flex items-center gap-1.5 rounded-xl bg-rose-600 px-2.5 py-1.5 text-[11px] font-bold text-white">
              <Ban {...iconProps} />
              Cancelar orden
            </button>
          </div>
