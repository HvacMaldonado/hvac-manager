  ...
  return (
    <div className="min-h-screen w-full max-w-full overflow-x-hidden bg-[radial-gradient(circle_at_top_left,_#0f172a55,_transparent_30%),radial-gradient(circle_at_top_right,_#0284c733,_transparent_28%),radial-gradient(circle_at_bottom_left,_#1e40af2e,_transparent_30%),linear-gradient(135deg,_#e2e8f0_0%,_#cbd5e1_48%,_#94a3b8_100%)] text-slate-900">
      ...
      <main className="w-full min-w-0 px-2.5 2xl:px-6 py-3 2xl:py-5">
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center justify-center bg-slate-950/55 p-4 backdrop-blur-sm">
            <div className="w-full max-w-md overflow-hidden rounded-3xl border border-white/20 bg-white shadow-2xl shadow-slate-950/40">
              ...
              <div className="grid gap-3 p-5">
                ...
                <button onClick={() => abrirProgramarCitaConCliente(clienteAccion)} className="flex items-center justify-center justify-center gap-2 rounded-2xl bg-gradient-to-r from-indigo-600 to-violet-600 px-4 py-3 font-black text-white shadow-lg">
        ...
        {session.role === "admin" && (
          <>
            <nav className="sticky top-[66px] 2xl:top-[92px] z-10 mb-3 grid w-full grid-cols-9 gap-1.5 rounded-2xl border border-slate-700/30 bg-slate-950/85 p-1.5 shadow-lg shadow-slate-900/25 backdrop-blur">
...
function Info({ titulo, valor, extra = "bg-white", icon: Icon = FileText }) { return <div className={`rounded-xl border p-2 text-xs ${extra}`}><p className="flex items-center justify-center gap-2 text-slate-400"><Icon {...iconProps} />{titulo}</p><p className="truncate font-bold">{valor}</p></div>; }
...
function FotoUploader({ titulo, imagen, onChange }) { return <div className="rounded-xl 2xl:rounded-2xl border bg-white p-2 2xl:p-3 text-sm 2xl:text-base"><p className="mb-2 text-sm font-bold">{titulo}</p>{imagen ? <img src={imagen} alt={titulo} className="mb-2 h-20 w-full rounded-xl border object-cover" /> : <div className="mb-2 flex h-20 w-full items-center justify-center justify-center rounded-xl border border-dashed bg-slate-50 text-sm text-slate-400">Sin foto</div>}<input type="file" accept="image/*" onChange={(e) => onChange(e.target.files[0])} className="w-full text-xs" /></div>; }
function ShareButtons({ orden, compartirOrden, t }) {
  return (
    <div className="flex max-w-full flex-wrap gap-1.5 2xl:gap-2 h-fit">
      <button onClick={() => compartirOrden(orden, "imprimir")} className="flex items-center justify-center gap-2 rounded-xl 2xl:rounded-2xl bg-gradient-to-r from-slate-950 to-blue-900 px-2 2xl:px-3 py-1.5 2xl:py-2 text-xs 2xl:text-sm font-bold text-white shadow-md shadow-slate-300">
        <Printer {...iconProps} />
        Imprimir
      </button>
          ...
          {tabs.map((item) => {
            ...
            return (
              <button
                ...
                className={
                  "flex w-full items-center justify-center gap-2 rounded-xl px-3 py-2 text-xs font-black transition " +
                  (active
