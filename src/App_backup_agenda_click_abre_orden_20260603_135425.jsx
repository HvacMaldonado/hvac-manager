...
function TecnicoAssignedTodayPanel({ ordenes = [], citas = [], obtenerCliente, ordenProps }) {
  const hoy = todayDateKey();
  ...
  const ordenarPorHora = (a, b) => {
    ...
    const horaA = normalizarHora(a.horaProgramada || a.hora);
    const horaB = normalizarHora(b.horaProgramada || b.hora);
  ...
  const citasHoy = citas
    .filter((cita) => esHoy(cita) && cita.estado !== "Convertida en orden")
    .sort(ordenarPorHora);

  const totalHoy = ordenesHoy.length + citasHoy.length;

  return (
    <section className="space-y-4">
      <div className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-lg shadow-slate-300/50">
          ...
          <div className="mt-2 flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
            <div>
              ...
              <p className="mt-1 text-sm font-bold text-white/60">
                Ordenado por hora. Desde aquí el técnico puede trabajar cada orden.
              </p>
            </div>
            ...
            <div className="rounded-2xl bg-white/10 px-4 py-3 text-right ring-1 ring-white/15">
              ...
              <p className="text-3xl font-black">{totalHoy}</p>
            </div>
          </div>
        </div>
