...
export default function TecnicoCompactOrderCard({ orden, cliente, ordenProps }) {
  ...
  return (
    <article className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm transition hover:-translate-y-0.5 hover:border-cyan-200 hover:shadow-xl">
      <div className="border-b border-slate-100 bg-gradient-to-br from-white via-slate-50 to-blue-50 p-4">
        <div className="flex items-start justify-between gap-3">
          ...
          <span className={`shrink-0 rounded-full border px-2.5 py-1 text-[10px] font-black ${flowState.tone}`}>
            {flowState.label}
          </span>
        </div>
      </div>
        ...
        <div className="grid grid-cols-2 gap-2 pt-1 sm:grid-cols-5">
          ...
          {!orden.horaLlegada && orden.estado !== "Trabajo en progreso" && orden.estado !== "Completado" && (
            <button
              ...
              className="inline-flex h-10 items-center justify-center gap-1 rounded-2xl bg-purple-700 px-3 text-xs font-black text-white shadow-sm"
            >
          ...
          {orden.horaLlegada && !orden.horaInicio && orden.estado !== "Completado" && (
            <div className="inline-flex h-10 items-center justify-center gap-1 rounded-2xl border border-purple-200 bg-purple-50 px-3 text-xs font-black text-purple-700 shadow-sm">
              <MapPinCheckInside size={13} />
              Esperando inicio
            </div>
          )}
          ...
          {!orden.horaInicio && orden.estado !== "Completado" && (
            <button
              ...
              className="inline-flex h-10 items-center justify-center gap-1 rounded-2xl bg-blue-700 px-3 text-xs font-black text-white shadow-sm"
            >
              <PlayCircle size={13} />
              Iniciar
            </button>
          ...
          {orden.horaInicio && orden.estado !== "Completado" && (
            <div className="inline-flex h-10 items-center justify-center gap-1 rounded-2xl border border-blue-200 bg-blue-50 px-3 text-xs font-black text-blue-700 shadow-sm">
              <PlayCircle size={13} />
              Trabajo en progreso
            </div>
          )}
          ...
          <button
            ...
            className="inline-flex h-10 items-center justify-center gap-1 rounded-2xl bg-slate-800 px-3 text-xs font-black text-white shadow-sm"
          >
            <PenLine size={13} />
          ...
          <button
            ...
            className="col-span-2 inline-flex h-10 items-center justify-center gap-1 rounded-2xl border border-rose-200 bg-rose-50 px-3 text-xs font-black text-rose-700 shadow-sm sm:col-span-5"
          >
            <Ban size={13} />
            Cancelar orden
          </button>
